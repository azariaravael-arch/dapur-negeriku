...
<h2 class="text-2xl font-bold text-[#5B6541] mb-6 ml-6 mobile:ml-0 mobile:text-lg mobile:mb-4">Profil Klien</h2>
        ...
        <form id="deleteForm" method="POST" class="p-6 mobile:p-4">
            ...
            <div class="text-center mb-6 mobile:mb-4">
                <p class="text-gray-600 mobile:text-sm" id="deleteMessage">
                    Data klien akan dihapus secara permanen
                </p>
            </div>
    ...
    <div class="flex items-center gap-1 mobile:flex-shrink-0">
        <div class="relative group">
            ...
            <input type="text" id="searchInput" name="search" value="{{ request('search') }}"
                ...
                placeholder="cari..." data-base-url="{{ route('klien.index') }}">
        ...
        <div class="relative">
            <div id="statusDropdown" class="relative cursor-pointer group">
                ...
                <div
                    ...
                    <form action="{{ route('klien.index') }}" method="GET" id="statusForm">
...
<div id="noResults" class="{{ $klien->count() > 0 ? 'hidden' : '' }} ml-6 mr-6 mobile:ml-4 mobile:mr-4 mb-4">
    <div class="bg-[#F7F6F2] border border-[#5B6541] shadow-lg rounded-lg p-8 text-center">
        <span class="material-icons text-[#5B6541] text-4xl mb-3">people</span>
        <h3 class="text-lg font-semibold text-[#5B6541] mb-2">Tidak ada klien ditemukan</h3>
        <p class="text-gray-600">
            @if(request('search'))
            Tidak ditemukan hasil untuk "<span class="font-medium">{{ request('search') }}</span>"
            @elseif(request('status'))
            Tidak ada klien dengan status "<span class="font-medium">{{ request('status') == '1' ? 'Aktif' : 'Tidak'
                }}</span>"
            @else
            Belum ada data klien yang ditambahkan
            @endif
        </p>
...
<!-- Table Container -->
<div class="ml-6 mr-6 mobile:ml-4 mobile:mr-4 mobile:overflow-x-auto {{ $klien->count() == 0 ? 'hidden' : '' }}">
    <!-- Table -->
        ...
        <table class="w-full text-left">
            <thead class="bg-[#5B6541] text-white">
                <tr>
                    ...
                    <th class="p-3 mobile:p-2 mobile:text-sm">Nama Klien</th>
            ...
            <tbody class="divide-y divide-[#5B6541]">
                ...
                <tr class="hover:bg-[#eceadd] transition-colors duration-200">
                    ...
                    <td class="p-3 pl-6 mobile:p-2 mobile:pl-4 text-[#5B6541] font-semibold mobile:text-sm">
                        {{ ($klien->currentPage() - 1) * $klien->perPage() + $loop->iteration }}
                    </td>

                    <!-- Foto -->
                    <td class="p-3 mobile:p-2">
                        <img src="{{ asset('storage/'.$item->foto_klient) }}"
                    ...
                    <td class="p-3 pr-6 mobile:p-2 mobile:pr-4">
                        <div class="flex gap-2 mobile:gap-1">
                            ...
                            <button type="button"
                                onclick="openDeleteConfirmation('{{ $item->id }}', '{{ $item->nama_klient }}')"
                                class="px-4 py-2 border border-[#5B6541] text-[#5B6541] rounded-lg text-sm flex items-center gap-1 hover:bg-[#5B6541] hover:text-white transition-all duration-200 hover:scale-105 active:scale-95 mobile:px-2 mobile:py-1 mobile:text-xs">
...
<div class="mt-6 flex justify-center px-6 mobile:px-4 mobile:mt-4 {{ $klien->count() == 0 ? 'hidden' : '' }}">
    @if ($klien->hasPages())
...
<div id="popupOverlay"
    ...
    <div
        ...
        <form id="addKlienForm" action="{{ route('klien.index') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10 mobile:gap-6">
                ...
                <!-- KIRI : FOTO dengan drag & drop -->
                <div>
                    <h4 class="text-lg font-semibold text-[#5B6541] mb-4 mobile:text-base flex items-center gap-2">
                        Foto Klien
                    </h4>

                    <div id="uploadArea" onclick="document.getElementById('fotoKlienInput').click()"
                    ...
                    <input type="file" id="fotoKlienInput" name="foto_klient" accept="image/*" class="hidden" required
                ...
                <div class="flex flex-col justify-between mobile:gap-6">
                    ...
                    <div>
                        ...
                        <div
                            ...
                            <input type="text" name="nama_klient" value="{{ old('nama_klient') }}"
...
<div id="editPopupOverlay"
    ...
    <div
        class="bg-[#F7F6F2] rounded-2xl shadow-2xl w-full max-w-4xl relative overflow-hidden border border-[#5B6541] mobile:rounded-xl mobile:max-h-[90vh] mobile:overflow-y-auto mobile:w-[95vw]">
        <form id="editKlienForm" method="POST" enctype="multipart/form-data" class="p-8 mobile:p-4">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10 mobile:gap-6">
                ...
                <div>
                    ...
                    <div id="editUploadArea" onclick="document.getElementById('editFotoKlienInput').click()"
                    ...
                    <input type="file" id="editFotoKlienInput" name="foto_klient" accept="image/*" class="hidden"
                ...
                <div class="flex flex-col justify-between mobile:gap-6">
                    ...
                    <!-- NAMA KLIEN -->
                    <div>
                        <label class="block text-lg font-semibold text-[#5B6541] mb-3 mobile:text-base">Nama Klien</label>
                        <div
                            class="border-2 border-dashed border-gray-300 rounded-xl p-1 mobile:p-0.5 hover:border-[#5B6541] transition-colors duration-300">
                            <input type="text" name="nama_klient" id="editNamaKlien"
        ...
        <div class="p-6 mobile:p-4">
            ...
            <div class="text-center mb-6 mobile:mb-4">
                <p class="text-gray-600 mobile:text-sm" id="cancelMessage">
                    Seluruh perubahan data klien tidak akan tersimpan
                </p>
            </div>
...
<script>
        ...
        function handleFiles(files, type) {
            if (files.length > 0) {
                ...
                if (file.type.startsWith('image/')) {
                    if (type === 'add') {
                        document.getElementById('fotoKlienInput').files = files;
                        previewImage({ target: document.getElementById('fotoKlienInput') });
                    } else {
                        document.getElementById('editFotoKlienInput').files = files;
                        previewEditImage({ target: document.getElementById('editFotoKlienInput') });
                    }
                } else {
    ...
    function clearAllFilters() {
        window.location.href = "{{ route('klien.index') }}";
    }
    ...
    function resetForm() {
        document.getElementById('addKlienForm').reset();
    ...
    function openEditPopup(id, namaKlien, fotoKlien, status) {
        currentPopupType = 'edit';
        // Set form action
        document.getElementById('editKlienForm').action = `/klien/${id}`;
        ...
        // Set foto klien preview
        const preview = document.getElementById('editImagePreview');
        const uploadIcon = document.getElementById('editUploadIcon');

        if (fotoKlien && fotoKlien !== '') {
            preview.src = fotoKlien;
            preview.classList.remove('hidden');
            uploadIcon.classList.add('hidden');
        } else {
    ...
    function openDeleteConfirmation(id, namaKlien) {
        deleteKlienId = id;
        deleteKlienName = namaKlien;

        // Set pesan konfirmasi
        document.getElementById('deleteMessage').textContent =
            `Apakah Anda yakin ingin menghapus klien "${namaKlien}"?`;

        // Set action form delete
        document.getElementById('deleteForm').action = `/klien/${id}`;
